...
def conv3x3(in_planes: int, out_planes: int, stride: int = 1, groups: int = 1, dilation: int = 1, ws: int = 224) -> nn.Conv2d:
    ...
    if stride == 1:
        ...
        return nn.Sequential(FLC_Pooling(ws),
                            nn.Conv2d(
                                in_planes,
                                out_planes,
                                kernel_size=3,
...
def conv1x1(in_planes: int, out_planes: int, stride: int = 1, ws: int = 224) -> nn.Conv2d:
    ...
    if stride == 1:
        ...
        return nn.Sequential(FLC_Pooling(ws), nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=1, bias=False))
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(FLC_Pooling(224), nn.Conv2d(3, self.inplanes, kernel_size=7, stride=1, padding=3, bias=False))
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = FLC_Pooling(112) # nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
